...
function contract() {
    ...
    document.querySelectorAll(".contract").forEach(contract => {
    
        const links = contract.querySelectorAll(".contract-links a");
        const documents = contract.querySelectorAll(".contract-documents > *");
    
        if(links.length <= 0 || documents.length <= 0) return false;
    
        links.forEach(link => {
            if(link.getAttribute("href") === new URL(location.href).hash) showDocument(link, links, documents);
            link.addEventListener("click", e=> showDocument(link, links, documents, e));
        });
    
    });
    
    function showDocument(link, links, documents, event) {
        ...
        links.forEach(link => (link.getAttribute("href") === targetIn) ? link.classList.add("active") : link.classList.remove("active"))
    
        documents.forEach(doc => ((targetIn.includes("all")) ? 
        doc.classList.remove("d-none") : (targetIn.includes(doc.id)) ? 
        doc.classList.remove("d-none") : doc.classList.add("d-none")));
    
        if(window.matchMedia("(max-width: 576px)").matches) {
            const headerFixed = document.querySelector(".fixed-header");
            window.scrollTo({top: document.querySelector(".contract-documents").offsetTop - headerFixed.offsetTop});
        } else {
            location.hash = '';
        };

        $(document).ready(() => history.pushState({}, "", targetIn));
...
window.addEventListener("resize", contract());
...
if(!document.cookie.includes("cookies=accepted") && !document.cookie.includes("cookies=rejected")) cookies({
    active: true,
    message: 'The website uses cookies to help personalize your internet experience. By accessing the Inexus website, you agree to the required <a href="terms-of-service.html#cookies" class="link">Cookie Terms</a>. We may use cookies to collect, store or track information for statistical and marketing purposes on our website.</p>',
});
...
function darkMode(isEnabled) {
    ...
    if(isEnabled) {
        ...
        if(darkModeTxt) darkModeTxt.innerHTML = 'Dark Mode';
